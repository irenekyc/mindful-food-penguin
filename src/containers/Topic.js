...
class Topic extends Component {
  constructor(props) {
    ...
    this.fetchData = async () => {
      const featureDetails = data.data.filter(
        (e) => e.features === this.props.match.params.topic
      );
    ...
    this.fetchRecipe = async () => {
      ...
      if (data.status === "failure") {
        return this.setState((prevState) => {
          return { ...prevState, recipeReady: true, dataError: true };
        });
      }
      const recipes = data.results;

      this.setState((prevState) => {
        return { ...prevState, recipes: recipes, recipeReady: true };
      });
    };
      ...
      <div>
        {this.state.dataReady ? (
          <TopicHeader
            ...
            filterClass={this.state.options}
          />
        ) : null}
